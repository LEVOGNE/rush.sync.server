...
# Always sync landing page and downloads (update on every rebuild)
WWW_DIR="/app/www/default-[8000]"
if [ -d "$WWW_DIR" ]; then
    cp /usr/local/share/landing.html "$WWW_DIR/index.html"
    cp /usr/local/share/README.md "$WWW_DIR/README.md" 2>/dev/null || true
    cp /usr/local/share/STARTUP.md "$WWW_DIR/STARTUP.md" 2>/dev/null || true
    cp /usr/local/share/BENCHMARK.md "$WWW_DIR/BENCHMARK.md" 2>/dev/null || true
    echo "[docker-entrypoint] Landing page synced to $WWW_DIR/"
